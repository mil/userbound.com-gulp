...
The above example<sup>1</sup> shows binding of the `add` action callback. In addition to `add` callback, the following callbacks may be bound to react to model changes:
...
<ol class='footnotes'>
    ...
    <li>The [demo code]() (76 SLOC) is only slighter heavier than the [Mmvp library]() itself (40 SLOC)</li>
</ol>
