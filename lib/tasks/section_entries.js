    ...
    gulp.task(collection_name + "_entries", function() {
      ...
      .pipe($.data(function(page_object) {
        ...
        if (collection_name == "blog") {
          ...
          page_object.vars.description = [page_object.vars.date, page_object.vars.time].join(" at ");
        }

        page_object.vars.path = [
          collection_name, 
          "/",
          util.source_filepath_to_url(page_object.history[1].split('/').pop()).split(".html")[0]
        ].join("");

        page_object.vars.collection_name = collection_name;
